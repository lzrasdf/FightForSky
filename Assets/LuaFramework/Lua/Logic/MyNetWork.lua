...
function MyNetwork.Receive(data)
    ...
    logWarn("MyNetwork.Receive " .. data)
    if true then
    	return
    end
...
function MyNetwork.FireProtocol( message )
	logWarn("MyNetwork.FireProtocolStart" .. message)
	...
	logWarn("MyNetwork.FireProtocolOver")
end

--按位读取协议数据
function MyNetwork.GetMsgData(typeId)
	logWarn("MyNetwork.GetMsgDataStart")
	local data = nil
	...
	logWarn("MyNetwork.GetMsgDataOver")
	return data
end
...
function MyNetwork.SendMsgData(protocol_id , str_id , ... )
	...
	str_len = string.len(msg)
	logWarn(str_len .. "--" .. msg)
	msg = "0000000000110011" .. msg
